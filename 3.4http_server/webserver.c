...
这是实现一个HTTP服务器的多进程反应堆模式代码,主要功能包括:

1. 初始化反应堆ntyreactor,包含epoll和事件块。

2. 创建监听socket,添加到反应堆监听。

3. 定义接收回调recv_cb,发送回调send_cb,接受回调accept_cb。

4. 在accept_cb中处理新连接,注册读取回调。

5. 在recv_cb中读取HTTP请求,解析方法和资源。

6. 在send_cb中构造HTTP响应,读取文件发送。

7. 主循环中通过epoll监听事件并驱动回调。

8. 主进程创建多个监听端口,实现多进程模型。

9. 使用事件块链表动态扩展结构体容量。

10. 访问静态资源实现简单HTTP服务器。
...
这实现了一个多进程的HTTP服务器,通过反应堆模式处理大量并发连接。
*/
...
int recv_cb(int fd, int events, void *arg) {
	...
	if (len > 0) {
		...
		nty_event_set(ev, fd, send_cb, reactor);//发起send
...
int send_cb(int fd, int events, void *arg) {
	...
	if (len > 0) {
		...
		nty_event_del(reactor->epfd, ev);
		nty_event_set(ev, fd, recv_cb, reactor); //recv
