...
1. 初始化一个ntyreactor对象,包含epoll实例和事件数组。

2. 创建监听socket,并添加到reactor的epoll监听。

3. 定义了接收回调recv_cb和发送回调send_cb。

4. 在accept_cb中接收新连接,设置读取回调recv_cb并添加到epoll。

5. 在recv_cb中读取数据,设置发送回调send_cb并添加到epoll。

6. 在send_cb中发送数据,设置读取回调recv_cb并添加到epoll。

7. 主循环ntyreactor_run通过epoll监听socket事件,调用回调处理。

8. 这样通过回调函数和epoll实现了Reactor模式,可以高效处理多连接的事件响应。
...
这实现了一个简单的Reactor模式服务器,可以高效处理多客户端连接。通过多线程或进程可以进一步提升并发性能。
*/
